...
[//]: # (TODO here it would be nice to reference some statistics that proof less code it better)
...
The compiler copies the function body directly to the call site instead of doing a function call. This means no stack frame setup, no register spilling, no return jump. The JIT compiler can see the whole inlined code as a 

[//]: # (TODO what does it mean single unit and apply optimisations and why it wouldn't be possible - use more contretics)
single unit and apply optimizations like loop unrolling and SIMD that wouldn't be possible across function boundaries. This matters a lot when you have filtering functions being called millions of times per second.
...
[//]: # (TODO what's cpu branch prediction and why you mention it here)
The type-level null safety eliminates runtime null checks in hot code paths, which means fewer branch instructions and better CPU branch prediction.

[//]: # (TODO what exactly let does under the hood, isn't it the same if block?)
...
[//]: # (TODO is domain the best word here?)
...
[//]: # (TODO here it would be nice to say how it would be done in java instead any why it's worse)
...
Here's what makes this efficient: the copy operation doesn't actually clone everything.

[//]: # (TODO so if i change the data from copied object it will also change the data in copy-object, as they share the reference?)
It only allocates new memory for the changed fields, while unchanged fields just share references to the existing data. 

[//]: # (TODO why 5 lines if it's only data keyword)
...
[//]: # (TODO here it would be nice to show how you can do exhaustive checking in vanilla java)
...
If you've been using the traditional Java approach with abstract classes and instanceof checks, those would have been 47 potential runtime errors with much higher discovery cost.
...
[//]: # (TODO here also check if it didn't change in java 25)
...
Java has the Stream API with operations like map, filter, and collect. 
These use eager evaluation, meaning they build intermediate lists for each operation. 

Kotlin has Sequences that use the same patterns but with lazy evaluation - the operations are only computed when you actually need the result.

[//]: # (TODO here it would be nice to have some real example)
For small lists, the difference doesn't matter much. 
But when you're working with large datasets or building processing pipelines, lazy evaluation with Sequences can be significantly more efficient.

One more feature of Kotlin is Destructuring. It is a syntax feature that lets you unpack objects. 
You can write `val (id, name) = user` and it unpacks the object in a single line, which is particularly useful when working with data classes.

---

## Slide 20-23: Functional Idioms

[//]: # (TODO here it would be nice to say how same could de achived in Java)
...
[//]: # (TODO what's namespace pollution)
...
[//]: # (TODO make this example abstract, so that it's not leaking real prod code. Explain how it would be done in Java alternatively and why Kotlin is better )
...
[//]: # (TODO is "real time" really a professional terminology here?)
...
You manually call onNext, onError, and onCompleted. 

[//]: # (TODO isn't there a way to track subs automatically in Java?)
You have to track subscriptions yourself for cleanup and handle error propagation explicitly across callbacks.
...
[//]: # (TODO where did you take those numbers? Insead use real ones from some company public report with link provided)
...
[//]: # (TODO here it would be nice to compare debugging in Intelij for both Java Threads and Kotlin Coroutines)
---
...
[//]: # (TODO but can't you do same in Java?)
You can also provide default parameters right in the constructor, like `channelBufferSize = 1_000`. 
...
[//]: # (TODO here it would be nice to explain what's Project Reactor and how it is connected to Java/Kotlin)
In Spring WebFlux with coroutines, you need to use specific annotations and sometimes wrap reactive types - the framework was built for Reactor first, coroutines second.

[//]: # (TODO why exactly does KTOR has less framework overhead)
Ktor also has less framework overhead - faster startup times and a smaller memory footprint. 
For a simple microservice that's mostly routing requests and calling other services, you don't need all of Spring's features, and Ktor's simpler architecture means there are fewer abstraction layers to go through.
...
[//]: # (TODO is Ktor Coroutines native? Is Spring?)
...
[//]: # (TODO make this whole chapter comparing to Java. Is ecosystem less/more reach?)
...
[//]: # (TODO here rather say what you didn't say and why. There are many Kotlin features left onexplained)
